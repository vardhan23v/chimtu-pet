...
enum Sprites {
    ...
    static let log = Logger(subsystem: "com.vardhan.chimtu", category: "sprites")

    /// (name, frame count, fps) — the single source of truth for the macOS app.
    static let specs: [(String, Int, Double)] = [
    ...
    /// Frames live in `frames/<skin>/`; the default skin also works from a flat `frames/` folder.
    private static func frameURL(_ file: String, skin: String) -> URL? {
        Bundle.main.url(forResource: file, withExtension: "png", subdirectory: "frames/\(skin)")
            ?? (skin == "shiba" ? Bundle.main.url(forResource: file, withExtension: "png", subdirectory: "frames") : nil)
    }
    ...
    static func hat(_ name: String, skin: String = "shiba") -> CGImage? {
        guard let url = frameURL("hat_\(name)", skin: skin) ?? frameURL("hat_\(name)", skin: "shiba") else { return nil }
        return image(at: url)
    }

    /// Load every animation for a skin. Missing frames are logged, never fatal;
    /// an animation with zero frames is simply absent and `enter()` ignores it.
    static func load(skin: String = "shiba") -> [String: Animation] {
        var out: [String: Animation] = [:]
        for (name, count, fps) in specs {
            var frames: [CGImage] = []
            for i in 0..<count {
                let file = String(format: "%@_%02d", name, i)
                if let url = frameURL(file, skin: skin), let img = image(at: url) { frames.append(img) }
                else { log.error("missing frame \(file, privacy: .public) for skin \(skin, privacy: .public)") }
            }
